...
//        private readonly string _modelPath = "wwwroot/mlmodels/loyaltyModel.zip"; 
...
//                .Select(x =>
//                {
//                    double totalSpentAsDouble = (double)x.TotalSpent;
...
//                        TotalSpent = Math.Round((double)x.TotalSpent, 2),
...
//                  double monetary = Math.Round((double)c.Orders.Sum(o => o.Quantity * o.Product.UnitPrice), 2);
...
//        private static double MonetaryScore(double amount) => amount switch
//        {
//            >= 5000 => 100,
//            >= 3000 => 80,
//            >= 1000 => 60,
...
namespace BigDataOrdersDashboard.Controllers
{
    public class CustomerLoyaltyController : Controller
    {
        ...
        public IActionResult ItalyLoyaltyScore()
        {
            var loyaltScores = _context.Customers
                ...
                .Select(c => new
                {
                    CustomerName = c.CustomerName + " " + c.CustomerSurname,
                    TotalOrders = c.Orders.Count(),
                    // SQL tarafında zorlama yapmıyoruz, orijinal decimal tipinde çekiyoruz
                    DbSpent = c.Orders.Sum(o => o.Quantity * o.Product.UnitPrice),
                    LastOrderDate = c.Orders.Max(o => (DateTime?)o.OrderDate)
                })
                .AsEnumerable() // Veriler RAM'e indi, artık C# dünyasındayız
                .Select(x =>
                {
                    // Mantığına dokunmadan, alt taraftaki kodlarınla uyumlu olması için double'a burada çeviriyoruz
                    double TotalSpent = (double)x.DbSpent;
                    ...
                    // Buradaki TotalSpent artık bir double olduğu için switch yapın hatasız çalışacak
                    double monetaryScore = TotalSpent switch
                    {
                        >= 5000 => 100,
                        >= 3000 => 80,
                        >= 1000 => 60,
        ...
        public IActionResult ItalyLoyaltyScoreWithML()
        {
            ...
            var data = _context.Customers
              ...
              .Select(c =>
              {
                  ...
                  // .AsEnumerable() sonrasında olduğumuz için bu (double) dönüşümü artık hatasız çalışır
                  double monetary = Math.Round((double)c.Orders.Sum(o => o.Quantity * o.Product.UnitPrice), 2);
        ...
        // Yardımcı skor methodları değiştilmedi
        private static double RecencyScore(double days) => days switch
        {
            <= 30 => 100,
            <= 90 => 75,
